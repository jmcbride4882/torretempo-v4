...
const statusColors: Record<string, string> = {
  ...
  expired: 'bg-kresna-light text-kresna-gray-dark border-kresna-border',
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-charcoal sm:text-2xl">{t('admin.inspectorTokens.title')}</h1>
            <p className="text-sm text-kresna-gray">{t('admin.inspectorTokens.subtitle')}</p>
          </div>
        </div>
      ...
      >
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          ...
          <SelectContent className="rounded-xl border border-kresna-border bg-white shadow-sm">
            <SelectItem value="all" className="text-kresna-gray-dark">{t('admin.inspectorTokens.allStatus')}</SelectItem>
            <SelectItem value="active" className="text-kresna-gray-dark">{t('admin.inspectorTokens.active')}</SelectItem>
            <SelectItem value="expired" className="text-kresna-gray-dark">{t('admin.inspectorTokens.expired')}</SelectItem>
            <SelectItem value="revoked" className="text-kresna-gray-dark">{t('admin.inspectorTokens.revoked')}</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-kresna-gray">
            <span className="font-medium text-kresna-gray-dark">{tokens.length}</span> {t('admin.inspectorTokens.totalLabel')}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <div className="h-2 w-2 rounded-full bg-emerald-500" />
          <span className="text-kresna-gray">
            <span className="font-medium text-kresna-gray-dark">{activeCount}</span> {t('admin.inspectorTokens.activeLabel')}
          </span>
        </div>
        {expiredCount > 0 && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-kresna-gray">
              <span className="font-medium text-kresna-gray-dark">{expiredCount}</span> {t('admin.inspectorTokens.expiredLabel')}
            </span>
          </div>
        )}
        {revokedCount > 0 && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-kresna-gray">
              <span className="font-medium text-kresna-gray-dark">{revokedCount}</span> {t('admin.inspectorTokens.revokedLabel')}
            </span>
          </div>
        )}
      </div>
          ...
          {generatedToken ? (
            <div className="space-y-4">
              <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4">
                ...
                <p className="mb-3 text-xs text-kresna-gray">
                  {t('admin.inspectorTokens.copyTokenWarning')}
                </p>
                <div className="flex items-center gap-2">
                  <code className="flex-1 overflow-hidden text-ellipsis rounded bg-kresna-light px-3 py-2 font-mono text-sm text-charcoal">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select value={selectedOrgId} onValueChange={setSelectedOrgId}>
                    ...
                    <SelectContent className="rounded-xl border border-kresna-border bg-white shadow-sm">
                      {tenants.map((tenant) => (
                        <SelectItem key={tenant.id} value={tenant.id} className="text-kresna-gray-dark">
                          {tenant.name}
                        </SelectItem>
                      ))}
                    </SelectContent>
                ...
                <div>
                  ...
                  <Select value={expiresInDays} onValueChange={setExpiresInDays}>
                    ...
                    <SelectContent className="rounded-xl border border-kresna-border bg-white shadow-sm">
                      <SelectItem value="7" className="text-kresna-gray-dark">{t('admin.inspectorTokens.days', { count: 7 })}</SelectItem>
                      <SelectItem value="14" className="text-kresna-gray-dark">{t('admin.inspectorTokens.days', { count: 14 })}</SelectItem>
                      <SelectItem value="30" className="text-kresna-gray-dark">{t('admin.inspectorTokens.days', { count: 30 })}</SelectItem>
                      <SelectItem value="90" className="text-kresna-gray-dark">{t('admin.inspectorTokens.days', { count: 90 })}</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
...
function TokenCard({ token, onRevoke }: TokenCardProps) {
  ...
  return (
    <div
      className={cn(
        'group relative overflow-hidden rounded-xl border border-kresna-border bg-white shadow-sm transition-all duration-300',
    ...
    >
      <div className="relative p-5">
        ...
        <div className="mb-4 flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary-50 shadow-sm">
              <Key className="h-5 w-5 text-primary-600" />
            </div>
            <div className="min-w-0">
              <h3 className="truncate font-semibold text-charcoal">{token.organizationName}</h3>
              <p className="truncate text-sm text-kresna-gray">
        ...
        <div className="mb-4 space-y-2 text-sm">
          <div className="flex items-center justify-between">
            ...
            <span className="text-kresna-gray-dark">{token.createdBy}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="flex items-center gap-1.5 text-kresna-gray">
            ...
            <span className={cn(
              'font-medium',
              isActive && daysUntilExpiry <= 7 ? 'text-primary-600' : 'text-kresna-gray-dark'
            )}>
              {isActive ? (
          ...
          {token.lastUsedAt && (
            <div className="flex items-center justify-between">
              <span className="text-kresna-gray">{t('admin.inspectorTokens.lastUsed')}</span>
              <span className="text-kresna-gray-dark">
        ...
        {isActive && daysUntilExpiry <= 7 && (
          <div className="mb-4 flex items-center gap-2 rounded-lg border border-primary-200 bg-primary-50 px-3 py-2">
...
function TokenCardSkeleton() {
  return (
    <div className="animate-pulse rounded-xl border border-kresna-border bg-white shadow-sm p-5">
      <div className="mb-4 flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className="h-11 w-11 rounded-xl bg-kresna-light" />
          <div className="space-y-1.5">
            <div className="h-5 w-32 rounded bg-kresna-light" />
      ...
      <div className="border-t border-kresna-border pt-4">
        <div className="h-8 w-full rounded bg-kresna-light" />
      </div>
    </div>
  );
}
...
function EmptyState({
  ...
  if (hasFilters) {
    return (
      <div
        className="flex flex-col items-center justify-center rounded-xl border border-dashed border-kresna-border bg-kresna-light px-6 py-16 text-center"
      >
        ...
        <h3 className="mb-1 text-lg font-semibold text-charcoal">{t('admin.inspectorTokens.noMatchingTokens')}</h3>
  ...
  return (
    ...
    >
      ...
      <h3 className="mb-1 text-lg font-semibold text-charcoal">{t('admin.inspectorTokens.noTokens')}</h3>
      <p className="mb-6 max-w-sm text-sm text-kresna-gray">
        {t('admin.inspectorTokens.noTokensDescription')}
      </p>
      <div>
        <Button onClick={onGenerate} className="gap-2 rounded-lg bg-primary-600 text-white hover:bg-primary-500">
          <Plus className="h-4 w-4" />
