    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="empty-state">
          ...
          <p className="text-kresna-gray">{t('templates.noTemplates')}</p>
        </div>
      ) : (
          ...
          {templateList.map((tpl: any) => (
            <Card key={tpl.id} className="p-5">
              <div className="flex items-start justify-between mb-3">
                <h3 className="font-semibold text-charcoal">{tpl.name}</h3>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-kresna-gray-dark">{tpl.startTime} - {tpl.endTime}</span>
                </div>
                {tpl.breakMinutes && (
                  <p className="text-kresna-gray">{t('templates.breakMinutes', { minutes: tpl.breakMinutes })}</p>
                )}
                {tpl.role && (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div>
              <label className="text-sm font-medium text-kresna-gray-dark mb-1.5 block">{t('common.name')}</label>
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder={t('templates.namePlaceholder')} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm font-medium text-kresna-gray-dark mb-1.5 block">{t('templates.startTime')}</label>
                <Input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} />
              </div>
              <div>
                <label className="text-sm font-medium text-kresna-gray-dark mb-1.5 block">{t('templates.endTime')}</label>
                <Input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} />
              </div>
            </div>
            <div>
              <label className="text-sm font-medium text-kresna-gray-dark mb-1.5 block">{t('templates.break')}</label>
              <Input type="number" value={breakMinutes} onChange={(e) => setBreakMinutes(e.target.value)} min="0" max="120" />
            </div>
            <div>
              <label className="text-sm font-medium text-kresna-gray-dark mb-1.5 block">{t('templates.role')}</label>
