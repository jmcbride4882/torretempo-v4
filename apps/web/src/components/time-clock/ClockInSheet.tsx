      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="text-center">
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <h2 className="text-xl font-semibold text-charcoal">{t('clock.clockIn')}</h2>
          </div>
        ...
        <div className="bg-white border border-kresna-border shadow-sm rounded-xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-kresna-gray-dark">{t('clock.location')}</span>
            </div>
            {geoLoading ? (
          ...
          {isPermissionDenied ? (
            ...
            <div className="flex items-center gap-3 py-2">
              <div className="h-10 w-10 rounded-full bg-kresna-light flex items-center justify-center">
                <Loader2 className="h-5 w-5 text-emerald-600 animate-spin" />
              </div>
              <div>
                <p className="text-sm text-kresna-gray-dark">{t('clock.fetchingLocation')}</p>
                <p className="text-xs text-kresna-gray">{t('common.pleaseWait')}</p>
              </div>
            </div>
          ) : position ? (
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="text-kresna-gray">{t('clock.coordinates')}</span>
                <span className="text-kresna-gray-dark font-mono">
                  {position.coords.latitude.toFixed(5)}, {position.coords.longitude.toFixed(5)}
                </span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-kresna-gray">{t('clock.accuracy')}</span>
                <span className={cn(
                  "font-mono",
                  hasLowAccuracy ? "text-amber-600" : "text-kresna-gray-dark"
                )}>
          ...
          <div className="bg-white border border-kresna-border shadow-sm rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-kresna-gray-dark">{t('clock.nfcTag')}</span>
              </div>
              {nfcScanning ? (
                <Badge variant="ghost" className="gap-1">
                  <Loader2 className="h-3 w-3 animate-spin" />
            ...
            {nfcError ? (
              <div className="flex items-start gap-3 p-3 rounded-lg bg-red-50 border border-red-200">
                ...
                <div>
                  ...
                  <p className="text-xs text-kresna-gray mt-1">{nfcError}</p>
                </div>
              </div>
            ) : nfcScanning ? (
              <div className="flex flex-col items-center justify-center py-6 gap-3">
                ...
                <p className="text-sm text-kresna-gray-dark">{t('clock.holdNfcNearDevice')}</p>
              </div>
            ) : nfcData ? (
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-kresna-gray">{t('clock.serialNumber')}</span>
                  <span className="text-kresna-gray-dark font-mono">{nfcData.serialNumber.substring(0, 16)}...</span>
                </div>
                {nfcData.message && (
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-kresna-gray">{t('common.message')}</span>
                    <span className="text-kresna-gray-dark">{nfcData.message}</span>
                  </div>
                )}
              </div>
            ) : !nfcSupported ? (
          ...
          <div className="bg-white border border-kresna-border shadow-sm rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-kresna-gray-dark">{t('clock.qrCode')}</span>
              </div>
              {qrScanning ? (
                <Badge variant="ghost" className="gap-1">
                  <Loader2 className="h-3 w-3 animate-spin" />
            ...
            {qrError ? (
              <div className="flex items-start gap-3 p-3 rounded-lg bg-red-50 border border-red-200">
                ...
                <div>
                  ...
                  <p className="text-xs text-kresna-gray mt-1">{qrError}</p>
                </div>
              </div>
            ) : qrScanning ? (
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-kresna-gray">{t('clock.location')}</span>
                  <span className="text-kresna-gray-dark">{qrData.locationName || 'Unknown'}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-kresna-gray">Location ID</span>
                  <span className="text-kresna-gray-dark font-mono">{qrData.locationId}</span>
                </div>
              </div>
            ) : !qrSupported ? (
              <div className="flex items-start gap-3 p-3 rounded-lg bg-amber-50 border border-amber-200">
          ...
          <div className="bg-white border border-kresna-border shadow-sm rounded-xl p-4 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-kresna-gray-dark">{t('clock.enterPin')}</span>
              </div>
            </div>

            <div className="space-y-3">
              <p className="text-xs text-center text-kresna-gray">
        ...
        {/* Clock Method Tabs */}
        <div className="space-y-2">
          <Label className="text-kresna-gray">{t('clock.clockMethod')}</Label>
            ...
            {CLOCK_METHODS.map((method) => {
              ...
              return (
                ...
                >
                  ...
                  {isDisabled && (
                    <span className="text-[10px] text-kresna-gray">{t('common.soon')}</span>
                  )}
                </button>
              );
            })}
        ...
        {/* Notes */}
        <div className="space-y-2">
          <Label htmlFor="clock-in-notes" className="text-kresna-gray">
        ...
        {success && (
          <div className="flex flex-col items-center justify-center py-4 gap-2">
            ...
            <p className="text-lg font-semibold text-charcoal">{t('clock.clockedInSuccess')}</p>
