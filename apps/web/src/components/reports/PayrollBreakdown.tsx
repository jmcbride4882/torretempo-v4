...
function TableRow({
  ...
  const rowStyles = {
    default: 'text-kresna-gray-dark',
    highlight: 'text-amber-700 bg-amber-50',
    deduction: 'text-red-700',
    total: 'text-charcoal font-semibold text-lg border-t border-kresna-border pt-3 mt-1',
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 text-right">
        {hours !== undefined && (
          <span className="min-w-[60px] font-mono text-sm text-kresna-gray">
            {formatHours(hours)}
          </span>
        )}
        <span
...
function SectionHeader({ title, icon: Icon }: { title: string; icon: typeof Calculator }) {
  return (
    <div className="mb-3 flex items-center gap-2 border-b border-kresna-border pb-2">
      <Icon className="h-4 w-4 text-kresna-gray" />
      <h4 className="text-xs font-medium uppercase tracking-wider text-kresna-gray">{title}</h4>
    </div>
  );
}
    ...
    >
      ...
      <div className="rounded-xl border border-kresna-border bg-white p-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-charcoal">{employee.name}</h3>
            <p className="text-sm text-kresna-gray">{employee.email}</p>
            <p className="text-xs text-kresna-gray capitalize">{employee.role}</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-kresna-border bg-white p-4 shadow-sm">
        ...
        <div className="space-y-1">
          ...
          <TableRow
            ...
            icon={Wallet}
            className="mt-2 border-t border-kresna-border pt-3"
          />
        </div>
      </div>
      ...
      <div className="rounded-xl border border-kresna-border bg-white p-4 shadow-sm">
        ...
        <div className="space-y-1">
          ...
          <TableRow
            ...
            icon={Minus}
            variant="deduction"
            className="mt-2 border-t border-kresna-border pt-3 font-medium"
          />
        </div>
      </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-kresna-gray">{t('reports.takeHomeAmount')}</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-xl border border-kresna-border bg-kresna-light p-3 text-center">
          <p className="text-xs text-kresna-gray">{t('reports.totalHoursSummary')}</p>
          <p className="text-lg font-bold text-charcoal">
            {formatHours(compensation.baseHours + compensation.overtimeHours)}
          </p>
        </div>
        <div className="rounded-xl border border-kresna-border bg-kresna-light p-3 text-center">
          <p className="text-xs text-kresna-gray">{t('reports.overtimeSummary')}</p>
          <p className="text-lg font-bold text-amber-600">
            {formatHours(compensation.overtimeHours)}
          </p>
        </div>
        <div className="rounded-xl border border-kresna-border bg-kresna-light p-3 text-center">
          <p className="text-xs text-kresna-gray">{t('reports.deductionRate')}</p>
