...
export function LocationFilter({
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="start"
        className="w-56 overflow-hidden rounded-xl border border-kresna-border bg-white p-1"
      >
        <AnimatePresence mode="popLayout">
          ...
          {locations.length > 0 && (
            <DropdownMenuSeparator className="my-1 bg-kresna-light" />
          )}
            ...
            >
              ...
              >
                ...
                <div className="flex-1 overflow-hidden">
                  ...
                  {location.address && (
                    <p className="truncate text-xs text-kresna-gray">{location.address}</p>
                  )}
                </div>
                {selectedLocationId === location.id && (
                  <motion.div
          ...
          {locations.length === 0 && !isLoading && (
            ...
            >
              <MapPin className="mx-auto h-8 w-8 text-kresna-gray" />
              <p className="mt-2 text-sm text-kresna-gray">{t('roster.noLocationsFound')}</p>
              <p className="text-xs text-kresna-gray">{t('roster.addLocationsInSettings')}</p>
            </motion.div>
          )}
        </AnimatePresence>
      </DropdownMenuContent>
